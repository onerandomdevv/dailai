...
exports.handleVoice = async (req, res) => {
    ...
    if (recordingUrl) console.log(`[Audio] Recording URL: ${recordingUrl.substring(0, 50)}...`);
    ...
    if (!rateLimiter.isAllowed(phoneNumber)) {
        ...
        return res.send(`<?xml version="1.0" encoding="UTF-8"?><Response><Say>Sorry, you have reached your daily limit for DialAI. Goodbye.</Say></Response>`);
    }
    ...
    try {
        ...
        // 3. Process Recording
            ...
            // Select Prompt
            let promptText = process.env.PROMPT_VOICE || "You are a helpful assistant. Speak briefly.";
            if (mode === "translator") {
                promptText = process.env.PROMPT_VOICE_TRANSLATOR || "You are a translator. Translate the audio strictly.";
            }
